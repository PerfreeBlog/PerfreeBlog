var form, xmSelect, element,categorySelect,tagSelect,toast,layer;
layui.use(['form', 'xmSelect', 'element','toast'], function () {
...
function submitArticle(data) {
    ...
    $.ajax({
        ...
        success: function (d) {
            if (d.code === 200) {
                parent.toast.success({message: "文章修改成功",position: 'topCenter'});
                parent.layui.admin.toPage('/admin/article', '', '', true);
                parent.layui.admin.closeTab('updateArticle');
            } else {
                parent.toast.error({message: d.msg,position: 'topCenter'});
            }
        },
        error: function (data) {
            parent.toast.error({message: "文章修改失败",position: 'topCenter'});
        }
    });
}
