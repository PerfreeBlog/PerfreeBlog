	...
	function(exports) {
		...
		var pearAdmin = new function() {
			...
			this.menuRender = function(param) {
				sideMenu = pearMenu.render({
					...
					change: function() {
						...
						sideMenu.selectItem(sideMenu.option.data[0].id);
			...
			this.bodyRender = function(param) {
                ...
                param.tab.index.id = sideMenu.option.data[0].id;
                param.tab.index.href = sideMenu.option.data[0].url;
                param.tab.index.title = sideMenu.option.data[0].name;
			...
			this.addTab = function(id, title, url, isRefresh = false) {
			...
			this.jump = function(id, title, url, isRefresh = false) {
				if (isMuiltTab(config) === "true" || isMuiltTab(config) === true) {
					pearAdmin.addTab(id, title, url, isRefresh)
				} else {
					pearAdmin.changeIframe(id, title, url)
				}
			}
			this.toPage = function(url,id, name, isRefresh = false) {
				var menu = '';
				menu = getMenuByUrl(sideMenu.option.data, url, menu);
				if (menu && menu !== '') {
					pearAdmin.jump(menu.id, menu.name, url, isRefresh);
				} else {
					pearAdmin.addTab(id, name, url, isRefresh);
				}
			}
		...
		function getMenuByUrl(menuList,url, menu) {
			for (let i = 0; i < menuList.length; i++) {
				if (menuList[i].url && menuList[i].url === url) {
					menu = menuList[i];
					return menu;
				}
				if (menuList[i].childMenu && menuList[i].childMenu.length > 0) {
					menu = getMenuByUrl(menuList[i].childMenu, url, menu);
				}
			}
			return menu;
		}
