...
layui.define(['table', 'jquery', 'element'], function (exports) {
		...
		$.each(option.data, function (i, item) {
			...
			if (item.childMenu && item.childMenu.length > 0) {
				...
				content += '<a  href="javascript:;" menu-type="' + item.type + '" menu-id="' + item.id + '" href="' + href +
					'" ' + target + '><i class="fa ' + item.icon + '"></i><span>' + item.name +
					'</span></a>';
			} else {
				content += '<a class="' + className + '" menu-type="' + item.type + '" menu-url="' + item.url + '" menu-id="' +
					item.id +
					'" menu-title="' + item.name + '"  href="' + href + '"  ' + target + '><i class="fa ' + item.icon +
	...
	function createMenuAndControl(option) {
		...
		$.each(option.data, function (i, item) {
			...
			index++;
			$.each(item.childMenu, function (i, note) {
				// 创 建 每 一 个 菜 单 项
				var content = '<li class="layui-nav-item" >';
				var href = "javascript:;";
				var target = "";
				...
				if (note.childMenu && note.childMenu.length > 0) {
					// 创 建 目 录 结 构
					content += '<a  href="' + url + '" ' + target + ' menu-type="' + note.type + '" menu-id="' + note.id +
						'" ><i class="fa ' + note.icon + '"></i><span>' + note.name +
						'</span></a>';
				} else {
					// 创 建 菜 单 结 构
					content += '<a ' + target + ' class="' + className + '" menu-type="' + note.type + '" menu-url="' + note.url +
						'" menu-id="' + note.id +
						'" menu-title="' + note.name + '" href="' + href + '"><i class="fa ' + note.icon +
						'"></i><span>' + note.name + '</span></a>';
				}
	...
	function loadchild(obj) {
		...
		if (!obj.childMenu || obj.childMenu.length <= 0) {
			return "";
		}
		// 创 建 子 菜 单 结 构
			...
			$.each(obj.childMenu, function (i, note) {
				...
				if (note.childMenu && note.childMenu.length > 0) {
					...
					content += '<a ' + target + ' class="' + className + '" menu-type="' + note.type + '" menu-url="' + note.url +
						...
						'" ><i class="fa ' + note.icon + '"></i><span>' + note.name + '</span></a>';
				}
				// 加 载 子 项 目 录
				content += loadchild(note);
				// 结 束 当 前 子 菜 单
