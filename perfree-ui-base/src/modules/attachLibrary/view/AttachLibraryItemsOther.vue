  ...
  <div>
    ...
    <div class="table-box">
      <el-table :data="tableData" style="width: 100%;height:100%;" row-key="id" v-loading="loading" >
        <el-table-column prop="name" label="附件名称" min-width="140" show-overflow-tooltip/>
        <el-table-column prop="url" label="附件" min-width="200" show-overflow-tooltip>
    ...
    <el-dialog v-model="open" :title="title" :width="dialogWidth(600)" draggable>
      ...
      >
        <el-form-item label="附件" prop="url">
          <attach-select-input :attach-type="'other'" :enable-input="true" :placeholder="'请选择或输入附件地址'" v-model:model-value="addForm.url" @attach-select-change="attachSelectChange"></attach-select-input>
        </el-form-item>
    ...
    <el-dialog v-model="attachShow" :title="title" :width="dialogWidth(1000)" draggable destroy-on-close>
      <attach-select-panel @update:selected-attach="selectAttach" :max="99" :attach-type="'other'"></attach-select-panel>
      <template #footer>
        <span class="dialog-footer">
...
const addRule = reactive({
  ...
  url: [{required: true, message: '附件地址不能为空', trigger: 'blur'}],
});
