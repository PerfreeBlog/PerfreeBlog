  ...
  <div>
    <div class="search-box">
      <el-form :inline="true" :model="searchForm" class="demo-form-inline" ref="searchFormRef">
        <el-form-item label="图片名称">
          <el-input v-model="searchForm.name" placeholder="请输入图片名称" clearable/>
        </el-form-item>
        <el-form-item>
    ...
    <div class="table-box">
      <el-table :data="tableData" style="width: 100%;height:100%;" row-key="id" v-loading="loading" >
        <el-table-column prop="name" label="图片名称" min-width="160" show-overflow-tooltip/>
        <el-table-column prop="url" label="图片" min-width="80">
          <template v-slot="scope">
            <el-image style="width: 100%; max-height: 100%" :src="scope.row.url" :zoom-rate="1.2" :max-scale="7" :min-scale="0.2"
                      :preview-src-list="[scope.row.url]" :initial-index="4" v-if="scope.row.url"
                      append-to-body fit="cover" preview-teleported lazy>
        ...
        <el-table-column prop="userInfo.userName" label="创建人" min-width="140" show-overflow-tooltip/>
...
<!--      <div class="attach-items-list">
        <div v-for="item in tableData" class="attach-item">
          <el-image style="width: 100%; max-height: 100%" :src="item.url" :zoom-rate="1.2" :max-scale="7" :min-scale="0.2"
                    :preview-src-list="[item.url]" :initial-index="4" v-if="item.url"
                    append-to-body fit="cover" preview-teleported></el-image>
          <div class="attach-item-name">{{item.name}}</div>
          <div class="attach-item-description">{{item.description}}</div>
        </div>
      </div>-->
    ...
    <el-dialog v-model="open" :title="title" width="600px" draggable>
      ...
      >
        <el-form-item label="图片" prop="url">
          <attach-select-input :attach-type="'img'" :enable-input="true" :placeholder="'请选择或输入图片地址'" v-model:model-value="addForm.url" @attach-select-change="attachSelectChange"></attach-select-input>
        </el-form-item>
        <el-form-item label="图片名称" prop="name">
          <el-input v-model="addForm.name" placeholder="请输入图片名称"/>
        </el-form-item>
    ...
    <el-dialog v-model="attachShow" :title="title" width="900px" draggable destroy-on-close>
      <attach-select-panel @update:selected-attach="selectAttach" :max="99" :attach-type="'img'"></attach-select-panel>
      <template #footer>
        <span class="dialog-footer">
...
const addRule = reactive({
  name: [{required: true, message: '图片库名称不能为空', trigger: 'blur'}],
